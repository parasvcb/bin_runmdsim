...
import sys
if len(sys.argv)!=4:
    print ("please enter correct cmd 1. equilibration till 10 ns (10 conf files should be ready) or production (1 for former 2 for latter) 2: production run in ns (100 for latte 0 for former) 3:DoneProductionTime(if no dcd immediately available then enter time in ns else False)")
    sys.exit()
prog,runtype,time,doneTime=sys.argv
